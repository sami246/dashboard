        ...
        {activeMenu && (<>
          <div className=' flex justify-between items-center'>
            <Link to="/" onClick={handleClosedSideBar} className='items-center gap-3 ml-3 mt-4 flex text-xl font-extrabold tracking-tight dark:text-white text-slate-900'>
              <SiShopware /> <span>Sami's Shoppy</span>
            </Link>
            <TooltipComponent content='Menu' position="BottomCenter">
              {/* onClick changes the value of activeMenu dependent on what previous value was */}
              ...
              {links.map((item) => (
                <div key={item.title}>
                  ...
                  {item.links.map((link) => (
                    ...
                    key={link.name}
                    // on click of link remove sidebar for mobile devices
